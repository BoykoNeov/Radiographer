...
- **AmBe deferred from M5.** The (α,n) source was scoped for M5 but its ISO 8529-1 (Kluge–Weise) reference spectrum could not be sourced as a clean, citable, machine-readable table (the standard is paywalled; open refs are figures only). Per the no-fabrication discipline (cf. the beta Cross-Berger kernel), it is deferred rather than reconstructed from memory; the schema/engine/bridge are AmBe-ready (a data drop + build function + the 4.438 MeV source-γ line). Its construction-dependent neutron yield would be a nominal, order-of-magnitude term.
