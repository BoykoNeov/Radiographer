...
- **Spent-fuel decay heat / γ dose underestimate the first ~day (M7c).** The ~150-nuclide discharge vector omits sub-hour fission products, so the prompt very-short-cooling component is missing. Valid for the cooling regime (months → millennia, where Cs-137/Sr-90 set the plateau); the shipped grid points are validated against published decay heat at 10 and 100 years. **Spent-fuel neutron output (SF + (α,n), chiefly Cm-244) is NOT modeled in v1** — only γ/β/decay-heat are computed (the dataset carries `_SF`/`_AN`; a future hook).
