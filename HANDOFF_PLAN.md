...
8. **OPEN — intermittent JS-heap runaway right at the M6a→M6b boundary (`drive_browser.mjs`), root cause not yet found.** Caught live (2026-07-08): the gate's browser process pegs one CPU core and grows to 3.7+ GB resident JS heap, then either OOM-crashes (Playwright's lightweight `chrome-headless-shell` binary, V8 "last resort" GC at ~2.2 GB old-space) or hangs indefinitely (full bundled Chromium, no fixed ceiling — the renderer's JS thread appears fully monopolized, since even `page.waitForFunction`'s own Node-side 60 s timeout never fires, consistent with a synchronous loop starving the CDP pipe). Reproduces on a clean, freshly-launched, zero-contention bundled-Chromium profile — **not** browser-channel or system-Chrome-profile-contention as originally suspected (see `[[gate-chromium-channel-priority]]`, now superseded). The `[data-testid="curves-plot"]` "Too many auto-margin redraws" Plotly warning fires at this exact point in *every* run, pass or fail — refuted as the discriminator (identical in both), but consistent with something (plausibly a Svelte `$effect` that never reaches a fixed point) repeatedly calling `Plotly.react` and allocating each cycle until the heap runs away. Advisor-recommended next step: instrument in-page (`performance.memory.usedJSHeapSize` + a `Plotly.react`/effect-fire counter logged at each suite boundary), catch one passing and one hanging run, and diff the trajectories — not yet done. `channel: "chromium"` is landed as a strict improvement (isolated profile, no headless-shell OOM ceiling) but does **not** fix this; the gate remains intermittently unreliable until this is root-caused.
