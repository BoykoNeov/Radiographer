...
- **AmBe deferred from M5.** The (α,n) source was scoped for M5 but its ISO 8529-1 (Kluge–Weise) reference *spectrum* was **not sourced within this session** as a clean, citable, machine-readable bin table — the standard is paywalled and the open refs surfaced were figures only. Not exhaustively searched: Vega-Carrillo's 60-bin ISO-AmBe tabulations and IAEA-TECDOC-410/TRS-318 remain untried leads. Per the no-fabrication discipline (cf. the beta Cross-Berger kernel) it is deferred rather than reconstructed from memory; the schema/engine/bridge are AmBe-ready (a data drop + build function + the 4.438 MeV source-γ line). Its validation anchor IS in hand: AmBe H\*(10) = 391 pSv·cm² (ICRP-74, JANP-4-005). Its construction-dependent neutron yield would be a nominal, order-of-magnitude term.
...
- **Spent-fuel neutron output is spontaneous fission only — a LOWER BOUND (M9).** Earlier notes said "the dataset carries `_SF`/`_AN`"; that was wrong — the SCK-CEN CSV carries `_SF` and `_GSRC`, there is **no `_AN`** column. The SF neutron source IS now modeled: `_SF` is the spontaneous-**fission rate** (`_SF/_A` reproduces the SF branching ratio), so per-nuclide `yield_n = (_SF/_A)·ν̄` with the prompt ν̄ from the **IAEA NDS safeguards table** (Simakov/Verpelli/Otsuka, `SF_n-Yield_20150313`; JEFF-3.1/Holden); the source is the multi-parent `S(t)=Σ yield_n·A_n(t)` off the one Bateman solve, folded against the validated Cf-252 representative SF spectrum (H\*(10) flat over 0.5–6 MeV). Cross-validated SF-only: the Cm-244 yield (`(_SF/_A)·ν̄ ≈ 3.7e-6 n/decay ≈ 1.1e7 n/s/g`) matches the IAEA's own n-yield/specific-activity to ~2%. **(α,n) on the oxygen in UO₂ is NOT in the dataset and is unmodeled — typically tens-of-% of the SF source — so the neutron dose is a lower bound (the dangerous under-count direction), loudly caveated.** Minor SF emitters with no evaluated ν̄ (chiefly **Cm-246**, which grows in once Cm-244 (18 yr) decays at multi-century cooling) are not in the dose either; the engine surfaces the **dropped SF-rate fraction** at the evaluated cooling time (never silent). Neutron shielding remains unmodeled (the γ shield does not attenuate n, §6.3).
