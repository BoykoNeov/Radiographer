...
**Shield builder:** ordered stack of (material, thickness) layers; material list **radiation-type-aware** (warn on high-Z for beta → bremsstrahlung; steer neutron to hydrogenous). Outputs: numeric dose with/without shield + attenuation factor; **dose-vs-thickness graph** and **dose-vs-time graph**. **Beam probe (post-v1):** the same stack can also be tested against an **external** beam instead of the loaded inventory — a **specific photon line** (transmission, buildup B, depth in mfp, per-layer μ/ρ·μ·mfp, HVL/TVL of the detector-side material) or an **idealized X-ray tube spectrum** (kVp + inherent filtration in mm Al; quantity-weighted transmission, beam hardening as mean energy and HVL-in-Al in→out, incident-vs-transmitted spectrum), plus a transmission-vs-energy curve for the stack. Output is **transmission only, never an absolute dose rate** — a tube's mGy/mAs output is not in `data/` and is not invented; the user multiplies their own measured unshielded rate. Probing the stack, not adding a source: this stays inside the §2 shielding scope and does not extend the §8 catalog. See `docs/plans/beam-probe.md`.
...
### Post-v1 extensions (M8–M13, plus later self-contained additions) — shipped beyond the original M0–M7 plan
...
- **Beam probe — shield vs an external line / X-ray tube.** The shield stack can be tested as a *shield* rather than as a filter in front of the inventory: a monoenergetic line probe (reusing the γ path's own `stack_transmission` bit-for-bit, plus HVL/TVL and a per-layer breakdown) and an idealized Kramers X-ray-tube continuum (kVp + inherent filtration; hardening + dropped-incident fraction). Stateless bridge call — no solve, no handle. Transmission only, never an absolute rate. See `docs/plans/beam-probe.md`.
...
- **The X-ray tube beam probe is an IDEALIZED analytic spectrum, not a measured one (beam probe).** The tube mode of the shield builder generates a **Kramers thick-target** continuum (`N(E) ∝ (E_max − E)/E`, Kramers 1923 — the same model the M4 in-shield bremsstrahlung uses) shaped by the user's **inherent filtration in mm Al equivalent**, which also stands in for anode self-absorption (absent from Kramers' law). It carries **no tungsten K characteristic lines** (present above roughly 70 kVp) and no anode-angle detail: their energies/yields are not in `data/` and are not reconstructed from memory (the no-fabrication rule that deferred AmBe and the Cross-Berger kernel). Only a **tungsten** anode is offered — Mo/Rh mammography beams are characteristic-line dominated, where a continuum-only model would be qualitatively wrong, not merely imprecise. Consequence: the **shape** of transmission-vs-energy and the **beam-hardening trend** are trustworthy; the absolute **HVL in mm Al is indicative only** (the modelled beam is softer than a real one at the same nominal filtration — e.g. ~2.6 mm Al at 100 kVp / 2.5 mm Al) and is **not a QA or compliance figure**. The probe reports **transmission only** — never an absolute dose rate, since a tube's mGy/mAs output is not bundled data. **Below a material's ANS-6.4.3 buildup floor (15 keV; 30 keV for lead) the probe refuses to score rather than assuming B=1**: a line probe there is a loud error and the tube fold excludes those bins from both sides of the ratio, reporting the excluded share as the **dropped incident fraction**. Because the excluded photons are the softest — the ones a stack removes hardest — the quoted transmission is an **upper bound** on the whole beam's. See `docs/plans/beam-probe.md`.
...
7. **OPEN — vendor Pyodide + micropip wheels for a fully self-hosted (no-CDN, no-PyPI) boot?** Today `pyodide-boot.ts` CDN-injects the Pyodide runtime (jsdelivr) and `micropip.install("radioactivedecay")` pulls wheels from PyPI's package index on every boot; only the engine+data archive (`radiographer-runtime.zip`) is same-origin. Candidate: fold a vendored Pyodide `full/` distribution and the resolved wheel set into the existing `build-archive.mjs` pipeline so the whole boot is same-origin fetches — no network dependency beyond loading the app itself. Tradeoff: the current git-ignored runtime archive is 6.1 MB; Pyodide + numpy/scipy/sympy/mpmath (which already drags in unwanted matplotlib/pandas/Pillow/networkx, see `docs/plans/M6a-bootstrap.md` "Open questions / risks") would add on the order of 100+ MB, and a Pyodide version bump becomes "re-download and commit binaries" instead of an `indexURL` string edit. This also revisits M6a's load-bearing decision #3 (CDN-injected, not npm import) — a deliberate self-hosting change, not a bug fix. Deferred until offline capability or CDN-outage resilience is actually needed; not scheduled to any M-milestone.
8. **RESOLVED (2026-07-08, commit `b930a9f`) — the intermittent gate hang / renderer OOM was a TEST-HARNESS artifact, not an app memory leak.** Root cause: `drive_browser.mjs` drove the DAG hide/show toggle with `await page.evaluate(() => window.__CY__.getElementById("Cs-137").emit("tap"))` — the arrow **returns** `emit()`'s value, the **Cytoscape node collection**, a deeply cyclic object (node → core → every node/style/renderer/canvas). `page.evaluate` then serialized that whole graph back over CDP: **~2 GB and ~6 s per call**. Two such calls in the `runViews` tail (hide + restore) drove RSS from ~1.6 GB to a **~4.8 GB peak**; ~1 run in N tipped past the ceiling and the renderer was **silently OS-OOM-reaped** (no Playwright `page.on("crash")` event — that's why "add a crash listener" never caught it), leaving the then-**unguarded** tail awaiting a dead renderer forever = the 10-minute zombie. Every earlier hypothesis was wrong: **not** a Svelte `$effect` render storm (render counters moved +1 per op, not thousands), **not** WASM heap growth, **not** browser-channel/profile contention, and the "Too many auto-margin redraws" Plotly warning was a red herring. The app itself is fine — a real user click serializes nothing across a bridge, so users never hit this. **Fix:** statement-body evaluate (no return) on both `emit("tap")` calls → `emit tap` **5852 ms → 12 ms**, whole-gate peak RSS **~4800 MB → ~1720 MB**, gate passes. **Robustness:** the four `runViews` tail ops are now wrapped in `guarded()` (25 s hard timeout) so any future silent renderer OOM fails fast with a named op instead of a multi-minute zombie. **Method that cracked it:** OS-level RSS sampling (a separate PowerShell process; `performance.memory.usedJSHeapSize` is a fake constant 10.6 MB in headless Chrome-for-Testing and was useless) streamed every 2 s + `[guard]` op-boundary timestamps + per-await sub-op timing → isolated the entire cost to the single `emit("tap")` evaluate. See `[[gate-js-heap-runaway]]`. *Follow-up (optional, non-blocking): the diagnostic scaffolding kept in the gate as a regression canary — the RSS watcher's 2 s `[rss]` stream, the `__PERF__` counters, `startRssWatcher` — can be quieted or removed in a dedicated pass now that the bug is closed.*
