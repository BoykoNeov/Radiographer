...
def test_cf252_spectrum_matches_independent_maxwellian():
    ...
    assert np.allclose(frac, indep, rtol=1e-4, atol=1e-12), (
        "stored Cf-252 fractions drifted from Maxwellian"
    )
...
def test_cf252_mean_energy_and_lethargy_peak():
    d = ns.load("Cf-252")
    assert d["mean_energy_MeV"] == pytest.approx(2.13, abs=0.1), (
        "Cf-252 mean E ~2.13 MeV (lit 2.1–2.3)"
    )
    lo, hi, frac = ns.spectrum("Cf-252")
    reps = ns.representative_energies("Cf-252")
    e_peak = reps[max(range(len(frac)), key=lambda i: frac[i])]
    assert 1.0 <= e_peak <= 3.0, (
        f"Cf-252 lethargy spectrum peaks at {e_peak:.2f} MeV (expected ~1–2)"
    )
...
def test_ambe_spectrum_matches_independent_transcription():
    ...
    assert np.allclose(frac, indep, rtol=1e-9, atol=1e-12), (
        "stored AmBe fractions drifted from TRS-403"
    )
...
def test_ambe_mean_energy_and_lethargy_peak():
    ...
    assert d["mean_energy_MeV"] == pytest.approx(3.5, abs=0.2), (
        "AmBe mean E ~3.5 MeV (ISO, truncated)"
    )
